...
export default function Teams() {
  const people = [
    ...
    },
    {
      ...
      twitterUrl: '#',
      linkedinUrl: '#',
      bioLink: 'https://soundcloud.com/byhaze?ref=clipboard&p=a&c=0&si=69c5003310114fb2ac07b839953e0afc&utm_source=clipboard&utm_medium=text&utm_campaign=social_sharing',
    },
          ...
          <ul role="list" className="space-y-4 sm:grid sm:grid-cols-2 sm:gap-6 sm:space-y-0 lg:grid-cols-3 lg:gap-8">
            {people.map((person) => (
              <Link href={person.bioLink}>
                <li key={person.name} className="cursor-pointer py-10 px-6 bg-gray-800 group hover:bg-white text-center rounded-lg xl:px-10 xl:text-left">
                  <div className="space-y-6 xl:space-y-10">
                    <img className="mx-auto h-40 w-40 rounded-full xl:w-56 xl:h-56 group-hover:scale-110" src={person.imageUrl} alt="" />
                    <div className="space-y-2 xl:flex xl:items-center xl:justify-between">
                      <div className="font-medium text-lg leading-6 space-y-1">
                        <h3 className="text-white group-hover:text-black">{person.name}</h3>
                        <p className="text-indigo-400">{person.role}</p>
                      </div>

                      <ul role="list" className="flex justify-center space-x-5 hidden">
                        <li>
